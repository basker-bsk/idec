...
export default function Carousel({ series }: any) {
  ...
  const settings: any = {
    ...
    infinite: false,
    speed: 500,
    slidesToShow: series?.crSlidesPerRow,
    slidesToScroll: 1,
    autoplay: false,
    autoplaySpeed: 5000,
    pauseOnHover: false,
  ...
  return (
    <>
      <div className="bg-gray-50">
        {series && (
          <div className=" mx-auto container  py-8 px-4 xl:py-12 xl:px-[50px]">
            <h3 className="mb-6 lg:mb-8">{series.crTitle}</h3>
            {ScreenSize() ? (
              <Slider {...settings}>
                {series?.crComponentsCollection?.items.map(
                  (slide: any, index: number) => (
                    <CarouselSlides props={slide} key={`b-${index}`} />
                  )
                )}
              </Slider>
            ) : (
